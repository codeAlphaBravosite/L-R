...
function renderChunk() {
  ...
  document.getElementById('chunkTiming').textContent =
...
function playCurrentChunk() {
  ...
  if (audioEl.readyState >= 1) {
    // Normal case (every chunk after the first): seek BEFORE playing.
    // Calling play() while currentTime is still sitting at the previous
    // chunk's position is exactly what caused audio to bleed in from the
    // previous chunk -- seeking first eliminates that window entirely.
    audioEl.currentTime = chunk.start;
    startPlayback();
  } else {
    // Only reachable on the very first tap of a session, before the audio
    // has finished loading metadata. Play immediately to preserve the
    // mobile tap-gesture attribution, then seek once ready.
    startPlayback();
  ...
  // Correct any residual seek drift, tightly and repeatedly, right as
  // playback begins. Seeking is not instantaneous -- especially on MP3,
  // which has no precise seek table -- so the position immediately after
  // play() starts can still be settling. A single loose check (1 second
  // tolerance) wasn't tight enough to catch smaller drifts that still
  // audibly blend with the previous chunk; this converges within ~240ms
  // using a much tighter 120ms tolerance, checked several times.
  const CORRECTION_TOLERANCE = 0.12;
  let correctionChecks = 0;
  const correctPosition = () => {
    if (audioEl.paused) return;
    if (Math.abs(audioEl.currentTime - chunk.start) > CORRECTION_TOLERANCE) {
      audioEl.currentTime = chunk.start;
    }
    correctionChecks += 1;
    if (correctionChecks < 6) setTimeout(correctPosition, 40);
  };
  setTimeout(correctPosition, 20);

  // High-precision stop: timeupdate only fires a few times per second
  // (commonly ~250ms apart), which lets playback overshoot a chunk's end
  // before we notice. requestAnimationFrame checks at display refresh
  // rate instead (~60 times/sec), and naturally adapts if playback speed
  // changes mid-chunk since it reads currentTime fresh every frame.
  const checkBoundary = () => {
    if (audioEl.paused) return; // stopped via another path (e.g. Next tapped)
    if (audioEl.currentTime >= chunk.end) {
      stopPlayback();
      return;
    }
    activeRafId = requestAnimationFrame(checkBoundary);
  };
  activeRafId = requestAnimationFrame(checkBoundary);
}
