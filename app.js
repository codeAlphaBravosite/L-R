...
function playCurrentChunk() {
  ...
  // Safety net: once playback genuinely starts producing audio, confirm
  // it actually landed at the right position. This is a second line of
  // defense in case any browser handles a seek set right around a play()
  // call differently than expected.
  const verifyPosition = () => {
    if (Math.abs(audioEl.currentTime - chunk.start) > 1.0) {
      audioEl.currentTime = chunk.start;
    }
    audioEl.removeEventListener('playing', verifyPosition);
  };
  audioEl.addEventListener('playing', verifyPosition);
