...
function updateDebugPanel() {
  ...
  const networkStateLabels = ['NETWORK_EMPTY', 'NETWORK_IDLE', 'NETWORK_LOADING', 'NETWORK_NO_SOURCE'];
  const err = audioEl.error;
  panel.textContent =
    `src set: ${!!audioEl.src}\n` +
    `readyState: ${audioEl.readyState} (${readyStateLabels[audioEl.readyState] || '?'})\n` +
    `networkState: ${audioEl.networkState} (${networkStateLabels[audioEl.networkState] || '?'})\n` +
    `duration: ${audioEl.duration}\n` +
    `currentTime: ${audioEl.currentTime}\n` +
    `paused: ${audioEl.paused}\n` +
    `muted: ${audioEl.muted}  volume: ${audioEl.volume}\n` +
    `error: ${err ? `code ${err.code}` : 'none'}`;
}
['loadedmetadata', 'canplay', 'canplaythrough', 'play', 'playing', 'pause', 'error', 'stalled', 'suspend', 'timeupdate']
  .forEach(evt => audioEl.addEventListener(evt, updateDebugPanel));
...
function playCurrentChunk() {
  ...
  updateDebugPanel();
  setTimeout(updateDebugPanel, 400);
